...
class DefaultAssetPickerBuilderDelegate<T extends DefaultAssetPickerProvider>
  ...
  Offset? _lastDragStartPosition;
  bool _isDragSelectionStarted = false;
                ...
                if ((dragToSelect ?? !accessibleNavigation) && !isSingleAssetMode) {
                  child = GestureDetector(
                    ...
                    onHorizontalDragStart: (d) {
                      _lastDragStartPosition = d.globalPosition;
                      _isDragSelectionStarted = false;
                    },
                    onHorizontalDragUpdate: (d) {
                      if (_isDragSelectionStarted) {
                      ...
                      if (_lastDragStartPosition != null) {
                        final double dx = (d.globalPosition.dx - _lastDragStartPosition!.dx).abs();
                        final double dy = (d.globalPosition.dy - _lastDragStartPosition!.dy).abs();
                        if (dx > dy) {
                          _isDragSelectionStarted = true;
                          dragSelectCoordinator.onSelectionStart(
                            context: context,
                            globalPosition: _lastDragStartPosition!,
                            index: index,
                            asset: assets[index],
                          );
                          dragSelectCoordinator.onSelectionUpdate(
